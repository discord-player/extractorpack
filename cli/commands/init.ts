...
export function initCommand(program: Command) {
    program
        ...
        .action(async (options: { force?: boolean }) => {
            if(options.force) console.warn(createWarnMessage("Using the --force flag. Recommended protections disabled"))
            if(!options.force && existsSync(path.join(process.cwd(), "extractorpack.config.mjs"))) {
